...
function calculate_correlations_callback(~, ~, f) 
    ...
    %% Combine histogram and heatmap into one figure
    combined_fig = figure('Name', 'IVCCA: Correlation Analysis', 'NumberTitle', 'off', 'Position', [100, 100, 800, 400]);
    iconFilePath = fullfile('Corr_icon.png');
    setIcon(combined_fig, iconFilePath);
    ...
    xtickangle(45); % Angle the x-axis labels for readability
...
%     setIcon(fig_gene_to_gene, iconFilePath);
    ...
    uitable(fig_gene_to_gene, ...
        ...
        'Position', [20, 20, 260, 400], ...
        'ColumnName', correlation_table.Properties.VariableNames,'ColumnSortable', true);

    % Create a bar plot for the sorted correlations
    ax = uiaxes(fig_gene_to_gene, 'Position', [300, 20, 400, 400]);
    bar(ax, correlation_table.Correlation, 'FaceColor', [0.2 0.2 0.8], 'EdgeColor', 'k'); % Use a single color for simplicity
    hold(ax, 'on');
    negative_indices = correlation_table.Correlation < 0;
    bar(ax, find(negative_indices), correlation_table.Correlation(negative_indices), 'FaceColor', 'r', 'EdgeColor', 'k'); % Highlight negative correlations

    % Add labels and formatting
    title(ax, sprintf('Correlation of %s to Selected Genes', single_gene_name));
    ylabel(ax, 'Correlation Coefficient');
    xticks(ax, 1:height(correlation_table));
    xticklabels(ax, correlation_table.Gene);
    xtickangle(ax, 45); % Angle the labels for readability

    % Include the average of the absolute correlations in the title
    avg_abs_correlation = mean(abs(correlation_table.Correlation));
    annotation(fig_gene_to_gene, 'textbox', [0.15, 0.85, 0.3, 0.1], 'String', ...
